...
STARTUP_LABEL = "Start on startup"
...
# Space kept between the account and the word beside it, and how far past a
# word a click still reaches it. A word is a smaller target than a mark, but it
# is also wider, so it needs less help.
ACCOUNT_GAP = 14
ACTION_HIT_PAD = 5
...
class Panel:
	...
	def _draw_account_row(self, y: float) -> float:
		...
		The name is fitted to whatever room the label on one side and the word
		on the other leave it, because an email address is as long as it is and
		the panel is not going to widen for it.
		...
		pad = self._unit(PANEL_PAD)
		right = self._unit(PANEL_WIDTH) - pad
		gap = self._unit(ACCOUNT_GAP)
		middle, height = self._draw_settings_label(ACCOUNT_LABEL, y, 0.0)

		action = SIGN_OUT_LABEL if self._signed_in else SIGN_IN_LABEL
		edge = self._draw_action(action, right, middle, ACCOUNT_KEY)

		name = self._account if self._signed_in and self._account else NOT_SIGNED_IN_LABEL
		room = edge - gap - (pad + self._font("row").measure(ACCOUNT_LABEL) + gap)
		self._text((edge - gap, middle), self._fit(name, "row", room), "row", LABEL_SECONDARY, anchor="e")
		return y + height
